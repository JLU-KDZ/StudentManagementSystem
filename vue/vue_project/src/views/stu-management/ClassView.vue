<template>
  <div id="app">
    ...
    <el-button type="primary" @click="addFormVisible = true"
      >新增班级</el-button
    >
    <el-dialog title="保存班级" :visible.sync="addFormVisible">
    ...
    <br />
    <!-- 表格 -->
    <el-table :data="classTable" border>
      <el-table-column
        type="index"
        :index="indexMethod"
        label="序列"
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button
            ...
            "
            >编辑</el-button
          ><el-dialog title="修改班级" :visible.sync="updateFormVisible">
...
export default {
  ...
  methods: {
    ...
    onAddSubmit() {
      ...
      try {
        if (this.addForm.name != "" && this.addForm.dept2Id != null) {
          ...
          //没插入 失败消息
          this.$message.error("错了哦，你必须添加班级名称、所属学院");
        }
      } catch (e) {
    ...
    onUpdateSubmit() {
      ...
      axios
        .put("http://localhost:8080/classes", this.updateForm)
        .then((result) => {
          console.log(result);
